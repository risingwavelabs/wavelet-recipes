...
5. The action appears in the dashboard with full visibility into automated decisions
...
2. Wait for the research team to hit $50 and the budget alert and agent action appear
3. Stop `seed.js` and updates stop immediately
4. Check `agent.js` terminal to see the closed-loop in action
5. Add a new query to `wavelet.config.ts` and it appears automatically after restart
